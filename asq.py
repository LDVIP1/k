...
def xoshnaw():
  ...
  try:
    ...
    if id in httpCaht:
      ...
      print("\x1b[1;91mID ACTIVATE (telegram) INBOX  @TYT2Y")
      os.system('xdg-open https://t.me/@TYT2Y')
      time.sleep(1)
      sys.exit()
  except:
    sys.exit()
...
def menu(my_name,my_id):
	...
	print(Z+'\x1b[38;5;208mTelegram ᯼ @TYT2Y  ')
	print(F+          'piad')
	print('[v-16]')
	print(f'»  Your ID : '+str(my_id))
	print(f'»  Your IP : {ip}')
	print('')
	print('» 1- Fishing from friends ✅✅  ')
	print('» 2- Fishing from followers  ☑️')
	print('» 3- Crack File 📥  ')		
	print('» 0- login out ❌   ')
...
#		method.append('touch')
	...
	if _jembot_ in ['']:
		...
		taplikasi.append('no')
	pwplus=input('>> Password Manual :  (T)( t ) ')
	if pwplus in ['y','Y']:
		pwpluss.append('ya')
...
def crack(idf,pwv):
	...
	fff = '%'
	print('\r%s <𝑴𝑻𝑬𝑹𝑬> %s/%s » [OK] %s » [CP] %s » %s%s%s'%(bi,loop,len(id2),ok,cp,int(pers),str(fff),x), end=' ');sys.stdout.flush()
